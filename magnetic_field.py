...
@jit(nopython=True)
def spline_fit_field_function(spline, position,max_dist,targetcenterz):
    '''
    ...
    a = len(spline) - (max_dist-(targetcenterz-0.5) - position_rounded) * math.pow(10, precision)
    #print(position, len(spline) - (max_dist - round(position, precision)) * math.pow(10, precision), spline[int(a)])
    return spline[int(a)]

@jit(nopython=True)
def max_step_length_fit_field_function(max_step_array, position,max_dist):
    '''
