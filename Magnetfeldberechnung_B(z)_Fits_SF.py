...
shift=250
dist=0#0.04
L = np.array([0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05])
N_coil=np.array([320,240,240,240,230,200,200,170,100,150])
L_field=0.58 + dist#+time for increase of  magnetic field
num = 5000
...
popt, pcov = curve_fit(B_coil, xnew+0.5, ynew,method="trf",bounds=(-20,10.7))
...
file=open("B(z)_0_5m_SF_long.txt","w+")
for i in range(len(y)):
    file.write("{};{}\n".format(x[i], y[i]))
file.close()
