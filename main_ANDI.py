...
from light_atom_interaction import doppler_shift, lorentzian_probability_2, laser_intensity_gauss
...
from magnetic_field import magnetic_field_polyfit, magnetic_field_function, magnetic_field_spline_fit, \
    spline_fit_field_function, max_step_length_fit_field_function, maximum_distance_setter
...
def timestep(atom_count, p_max, v_min, v_max, x_min, x_max, y_min, y_max, exciting_freq, time_step, wavelength,
    ...
    for i in range(0, atom_count):
        ...
        minimum_path_length = max_step_length_fit_field_function(max_step_fit_function, z_pos - target_center_z,maximum_distance)
        ...
        while atom_dead != 0:
            ...
            if z_pos < target_center_z + target_radius:
                ...
                if x_y_pos_component_squared < light_beam_radius_squared:
                    for a in range(len(quantum_numbers_excited_state)):
                        ...
                        if quantum_numbers_excited_state[a] - ground_state_quantum_numbders[current_groundstate] == 0 or \
                            ...
                            if z_pos > zeeman_distance:
                                zeeman_shift_freq = (-bohr_magnetron * (
                                            quantum_numbers_excited_state[a] * lande_factor_excited_state -
                                            ground_state_quantum_numbers[
                                                current_groundstate] * lande_factor_ground_state) * spline_fit_field_function(
                                    spline_fit, z_pos - target_center_z,maximum_distance,target_center_z)) / h_bar
                                frequency_shift_list[a] += zeeman_shift_freq
                            excitation_prob[a] = lorentzian_probability_2(frequency_shift_list[a], laser_frequency,
                                                                        laser_detuning, natural_line_width,
                                                                        laser_intensity_gauss(x_y_pos_component_squared,
                                                                                              z_pos, intensity),
                                                                        laser_saturation_intensity)
                            #print(lorentzian_probability_2(frequency_shift_list[a], laser_frequency,
                            #                                            laser_detuning, natural_line_width,
                            #                                           laser_intensity_gauss(x_y_pos_component_squared,
                            #                                                                  z_pos, intensity),
                            #                                            laser_saturation_intensity), frequency_shift_list[a], laser_frequency,
                            #                                            laser_detuning, natural_line_width,
                            #                                            laser_intensity_gauss(x_y_pos_component_squared,
                            #                                                                  z_pos, intensity),
                            #                                            laser_saturation_intensity)
                    ...
                    if spline_fit_field_function(spline_fit, z_pos - target_center_z,maximum_distance,target_center_z) < magnetic_field_cutoff:
                    ...
                    elif excitation_prob[current_groundstate] / excitation_prob_sum < random_number_for_lorentz < \
                            excitation_prob[current_groundstate + 1] / excitation_prob_sum:
                        excitation_index = current_groundstate + 1
                    elif excitation_prob[current_groundstate + 1] / excitation_prob_sum < random_number_for_lorentz < \
                            excitation_prob[current_groundstate + 2] / excitation_prob_sum:
                    ...
                    excitation_time_step = (
                                (-1 / ((excitation_probability + threshold) * (natural_line_width))) * math.log(
                            rnd_number + 1E-35))
                    ...
                    if excitation_time_step < initial_atom_time_step:
                        ...
                        x_velocity -= math.cos(random_number_theta) * math.sin(random_number_phi) * scc.h / (
                                    mass_lithium_6 * wavelength)
                        y_velocity -= math.sin(random_number_theta) * math.sin(random_number_phi) * scc.h / (
                                    mass_lithium_6 * wavelength)
                        ...
                        minimum_path_length = max_step_length_fit_field_function(max_step_fit_function,
                                                                                 z_pos - target_center_z,maximum_distance)
                        # print("Min path length: ", minimum_path_length, "Z position: ", z_pos)
                        initial_atom_time_step = minimum_path_length / (z_velocity + threshold)
                        # atom_time_step = initial_atom_time_step
...
if __name__ == '__main__':
    ...
    laser_saturation_intensity = 1/sim_param_data['slower_laser_saturation'] * intensity
    ...
    freq_offset=[float(i) for i in freq_offset]
    ...
    line_plotting(observing_z_position, observing_z_velocity, 'z position', 'z velocity', 0.0, 0.55, 0.0, 1005.0,
                  startTime, False)
