...
coils = 20
dist=0#0.04
L = np.array([0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05,0.05])
N_coil=np.array([320,290,260,260,270,260,250,250,240,230,220,190,190,160,160,140,130,130,100,100])
L_field=1.03 + dist#+time for increase of  magnetic field
num = 5000
...
popt, pcov = curve_fit(B_coil, xnew+0.5, ynew,method="trf",bounds=(-10,24.5))
...
file=open("B(z)_1_0m_changed.txt","w+")
for i in range(len(y)):
    file.write("{};{}\n".format(x[i], y[i]))
file.close()
