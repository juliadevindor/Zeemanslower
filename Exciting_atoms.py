...
def Excitation(Debug_flag, current_groundstate,counter,mass_lithium_6,GS_quantum_number,Bfield, z_pos, zeeman_distance, mean_free_path_bigger, max_step_fit_function, target_center_z, maximum_dist, vx,vy,vz,kx,ky,kz,wavelength,polarization,excitation_counter,mean_free_path_smaller,laser_frequency,x_y_pos_component_squared,laser_intensity,laser_detuning,natural_line_width,threshold):
    ...
    if Debug_flag==1 and z_pos>0.6: print("=====================================")
    if Debug_flag==1 and z_pos>0.6: print("zpos:", z_pos, "GS:", GS,"vz",vz)
    ...
    if z_pos > zeeman_distance:
        for a in range(0,12): #loop over all excited states
            ...
            if Debug_flag==1 and z_pos>0.6: print("exc rate sum", excitation_rate_sum)
    ...
    if Debug_flag==1 and z_pos>0.6: print("exc time step", excitation_time_step)
    maximum_path_length = max_step_length_fit_field_function(max_step_fit_function, z_pos - target_center_z,maximum_dist)
    if Debug_flag==1 and z_pos>0.6: print("max path length", maximum_path_length)
    atom_path_length = excitation_time_step*vz
    if Debug_flag==1 and z_pos>0.6: print("atom path length",atom_path_length)

    if atom_path_length < maximum_path_length:  # Scatter a photon!
        if Debug_flag==1 and z_pos>0.6: print("scatter")
        ...
        if Debug_flag==1 and z_pos>0.6: print("exc state", ES)
        ...
        if Debug_flag==1 and z_pos>0.6: print("deex_rate_sum",deex_rate_sum)
        ...
        if Debug_flag==1 and z_pos>0.6: print("groundstate",GS_quantum_number)
        # de-excitation
        ...
        if Debug_flag==1 and z_pos>0.6: print("noscatter")
        #if z_pos+maximum_path_length>0.18833766753350667+0.5 and z_pos<0.18833766753350667+0.5: #new
        #    atom_path_length = 0.68833766753350667-z_pos #new
        #else: #new
