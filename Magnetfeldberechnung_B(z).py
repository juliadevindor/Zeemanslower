...
N = np.array([647,415,386,362,336,311,268,266])
I = np.array([4.1,4.8,4.8,4.8,4.8,4.8,4.8,4.2])
# field like the one that has been measured
L = np.array([0.036, 0.036, 0.036, 0.036, 0.036, 0.036,0.036,0.036])  # real length values for Zeeman coils
...
for o in range(0, num):  # loop over z (along the beam axis)
    ...
    for j in range(0, coils):  # loop over Zeeman coils
        ...
        for mi in range(0, M[j]): # loop over all layers of each coil
            ...
            ##
            if j==0:
                B_1tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j], z[o])
            if j==1:
                B_2tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j],z[o])
            if j==2:
                B_3tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j],z[o])
            if j==3:
                B_4tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j],z[o])
            if j==4:
                B_5tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j],z[o])
            if j==5:
                B_6tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j],z[o])
            if j==6:
                B_7tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j],z[o])
            if j==7:
                B_8tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j], z[o])
            if j==8:
                B_9tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j], z[o])
            if j == 9:
                B_10tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j], z[o])
            if j == 10:
                B_11tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j], z[o])
            if j == 11:
                B_12tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j], z[o])
            if j == 12:
                B_13tot[o] += B_coil(I[j], N_wires[j], L[j], R_layer, z0[j], z[o])
            ##
...
print("Bmax",max(B_1tot*1e4),max(B_2tot*1e4),max(B_3tot*1e4),max(B_4tot*1e4),max(B_5tot*1e4),max(B_6tot*1e4),max(B_7tot*1e4),max(B_8tot*1e4))
