...
def load_files(sim_path, exp_path, atom_path, magn_path, max_step_path):
    ...
    # Save the fit as a pickle for later use in additional simulation runs with the same magnetic field.
        ...
        for i in range(len(spline_fit)):
            if i==0:
                ...
                spline_fit_x[i]=spline_fit_x[i-1]+1.3/len(spline_fit)
        #plt.plot(spline_fit_x,spline_fit)
        #plt.show()
        ...
        with open(magn_path) as magnetic_file:
            ...
            for i in range(len(distance_steps)):
                ...
                for j in range(i + 1, len(distance_steps)):
                    #print(i,j,b_initial*1e4,spline_fit[j]*1e4, (b_initial / (spline_fit[j] + 1E-35)),(((j - i) / 1E4) * distance))
    ...
    #print("1",spline_fit_field_function(spline_fit, (0.4999 - 0.5), max_distance))
